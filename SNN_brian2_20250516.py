...
class Model:
    ...
    def setup_network(self, debug=False):
        ...
        # Define separate scaling parameters for EG and IG
        self.g_scaling_EG = 10 * uS   # For excitatory group, based on I-V data of Jing's Device
        self.g_scaling_IG = 10 * uS   # For inhibitory group

        # Excitatory neuron group
        EG = NeuronGroup(
            self.n_e, '''
            dv/dt = ((ge + gi) * (0*mV - v) / g_scaling + (v_rest_e - v)) / tau_m_ex : volt
            dge/dt = -ge / tau_e : siemens
            dgi/dt = -gi / tau_i : siemens
            dv_th/dt = -v_th / tau_t_h : volt
            g_scaling : siemens (shared)
            ''',
            threshold='v > v_thresh_e + v_th',
            reset="v = v_reset_e; v_th += v_thresh_e_inc",
            refractory=self.refrac_exc,
            method='euler',
            name='EG'
        )
        EG.g_scaling = self.g_scaling_EG

        # Inhibitory neuron group
        IG = NeuronGroup(
            self.n_i, '''
            dv/dt = ((ge + gi) * (0*mV - v) / g_scaling + (v_rest_i - v)) / tau_m_in : volt
            dge/dt = -ge / tau_e : siemens
            dgi/dt = -gi / tau_i : siemens
            g_scaling : siemens (shared)
            ''',
            ...
            reset="v = v_reset_i",
            refractory=self.refrac_inh,
            method='euler',
            name='IG'
        )
        IG.g_scaling = self.g_scaling_IG

        # Initialize voltage as before
